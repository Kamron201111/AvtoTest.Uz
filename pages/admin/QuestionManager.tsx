              ...
              <div className="space-y-1.5 max-h-48 overflow-y-auto">
                {preview.map((q, i) => (
                  <div key={i} className="text-xs bg-white dark:bg-slate-700 rounded-lg p-2 border border-green-100 dark:border-green-900 flex items-center gap-2">
                    ...
                    {q.description && <span className="text-blue-400 flex-shrink-0">📖</span>}
                  </div>
                ))}
              </div>
            </div>
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-lg shadow-2xl">
        ...
        <div className="p-6 space-y-4">
          ...
          {status === "idle" && (
            <label className="flex flex-col items-center justify-center border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-xl p-8 cursor-pointer hover:border-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-all">
          ...
          {status === "error" && (
            <div className="text-center py-4">
              ...
              <label className="cursor-pointer px-6 py-2.5 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-all font-medium inline-block">
                Boshqa PDF tanlash
                <input type="file" accept=".pdf" onChange={handleFile} className="hidden" />
              </label>
            </div>
    ...
    <div className="max-w-4xl mx-auto p-4 sm:p-6 min-h-screen flex flex-col">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        ...
        <div className="flex gap-2 w-full sm:w-auto flex-wrap">
          ...
          <button onClick={() => navigate("/admin/questions/new")}
            className="flex-1 sm:flex-none bg-blue-600 text-white px-3 py-2 rounded-lg flex items-center justify-center gap-1.5 text-sm font-medium hover:bg-blue-700 transition-all">
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 mb-4 flex-shrink-0">
        {categories.map(cat => (
          <button key={cat.id} onClick={() => setSelectedCategory(cat.id)}
            className={`px-3 py-1.5 rounded-lg font-medium whitespace-nowrap transition-colors text-sm ${selectedCategory === cat.id ? "bg-blue-600 text-white shadow-md" : "bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700"}`}>
            {cat.name}
            {cat.id !== "all" && (
      ...
      <div className="relative mb-4 flex-shrink-0">
        ...
        <input type="text" placeholder={t("q_search")} value={search} onChange={e => setSearch(e.target.value)}
          className="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none text-slate-800 dark:text-white shadow-sm" />
      </div>
      ...
      <form onSubmit={handleSave} className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 space-y-5 transition-colors">
        ...
        <div>
          ...
          <div className="flex items-center gap-4">
            ...
            <label className="cursor-pointer bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-100 transition-colors">
              Rasm Tanlash
        ...
        <button type="submit" disabled={saving}
          className="w-full py-3 bg-blue-600 text-white rounded-xl font-bold shadow-lg hover:bg-blue-700 transition-all flex justify-center gap-2 disabled:opacity-50">
