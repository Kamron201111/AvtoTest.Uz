      ...
      <div className="min-h-screen bg-white dark:bg-slate-950 flex flex-col">
        ...
        <div className="px-4 py-4 flex gap-3 border-t border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900">
          ...
          <button onClick={() => next && setActiveLesson(next)} disabled={!next}
            className="flex-1 py-3.5 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-2xl font-bold flex items-center justify-center gap-2 disabled:opacity-30 transition-all shadow-lg">
          ...
          lessons.map(lesson => {
            ...
            return (
              <button key={lesson.id} onClick={() => { if (!locked) setActiveLesson(lesson); }}
                className={`w-full bg-white dark:bg-slate-900 rounded-2xl p-4 text-left border shadow-sm transition-all ${locked ? "border-slate-100 dark:border-slate-800 opacity-90" : "border-slate-100 dark:border-slate-800 hover:border-blue-300 dark:hover:border-blue-700 active:scale-[0.99]"}`}>
                {locked ? (
                  <div className="flex items-center justify-center py-1">
                    <div className="flex items-center gap-2 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-xl px-4 py-2.5 w-full justify-center">
