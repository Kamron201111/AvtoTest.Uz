    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/20 dark:from-slate-950 dark:via-slate-900 dark:to-slate-900">
      <div className="max-w-lg mx-auto px-4 pt-5 pb-4 space-y-4">
          ...
          <div className="flex items-center gap-2">
            {premiumInfo.active ? (
              ...
              <button
                ...
                className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white text-xs font-bold px-3 py-1.5 rounded-xl"
              >
                ⭐ Premium
              </button>
            )}
            ...
            <div className="flex items-center gap-2">
              <div className="flex gap-1">
                {Array.from({ length: Math.min(dailyInfo.limit, 10) }).map((_, i) => (
                  <div key={i} className={`w-2 h-4 rounded-full ${i < Math.min(dailyInfo.used, 10) ? "bg-blue-500" : "bg-slate-200 dark:bg-slate-600"}`} />
                ))}
              </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            { label: "material o'tildi", value: `${Math.min(avgScore, 100)}%`, color: "from-blue-500 to-blue-600", sub: history.length ? `${history.length} ta test` : "Hali yo'q" },
            { label: "platformada o'tkazildi", value: `${Math.round(history.reduce((s, r) => s + (r.timeSpentSeconds || 0), 0) / 3600 * 10) / 10} soat`, color: "from-indigo-500 to-purple-600", sub: "Umumiy vaqt" },
        ...
        <div>
          ...
          {history.length === 0 ? (
            ...
            <div className="space-y-2">
              ...
              {history.length > 3 && (
                <button onClick={() => navigate("/history")} className="w-full py-2.5 text-blue-600 text-sm font-bold text-center">
                  Hammasi ko'rish →
                </button>
              )}
            </div>
        ...
        {!premiumInfo.active && !pendingRequest && (
          <div className="bg-gradient-to-br from-blue-600 via-indigo-600 to-purple-700 rounded-3xl p-5 text-white shadow-xl relative overflow-hidden">
            ...
            <div className="relative">
              ...
              <p className="text-blue-200 text-xs mb-4">Barcha imkoniyatlarni oching</p>
              ...
              <button onClick={() => setPremiumStep("plans")} className="w-full py-3 bg-white text-blue-700 rounded-2xl font-black shadow-lg hover:bg-blue-50 transition-all">
